...
int main() {
    ...
    double Isat = parameters::pi * parameters::plancks_constant * parameters::speed_of_light * G_tot /
                  (3.0 * pow(transition_lambda, 3));
    cout << "Isat: " << Isat << endl;
    ...
    for (size_t D_index = 0; D_index < Ds.rows(); ++D_index) {
        for (size_t I_index = 0; I_index < Is.rows(); ++I_index) {
            ...
            int n_states = n_ground_states + n_excited_states;
            int size = n_states * params.n_x_max * params.n_z_max;

            MatrixXcd H0 = hamiltonian::build_H_zero_freq(H_ground, H_excited,
            ...
            for (const auto& g : G) num_G_nonzero_entries += (g.array() != 0.0).count();

            cout << "Size of G: " << num_G_nonzero_entries << endl;

            auto L = ss_spin::build_L(G, n_states, params.n_x_max, params.n_z_max,
                                      num_G_nonzero_entries, params.mass,
                                      params.omega_x, params.omega_z,
                                      transition_lambda, B_direction, seed);

            cout << "2. Finished Building L!" << endl;

            int N = 2001;
            VectorXd t = VectorXd::LinSpaced(N, 0.0, 100e-6);
            ...
            vector<unsigned int> keys(100);
            for (unsigned int i = 0; i < 100; ++i) keys[i] = i;

            auto [psi_final, jumps, nx_over_t, nz_over_t] =
                ss_spin::solve(t, psi0, H, L, n_states, params.n_x_max, params.n_z_max, keys);
